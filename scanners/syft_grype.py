...
class CustomScanner(Scanner):
    ...
    def scan(self, target, working_dir, outputs, network):
        ...
        try:
            ...
            self.logger.error(
                "Could't create output file in host %s", sbom_path_table)
            sys.exit()
        ...
        try:
            ...
            self.logger.error(
                "Could't create output file in host %s", sbom_path_json)
            sys.exit()
    ...
    def get_findings_count(self, json_file):
        severity_index={"Negligible":0,"Low":1,"Medium":2,"High":3,"Critical":4,"Unknown":5}
